...
* `cluster-agentname`: `config` is the cert ca/key/crt from the argocd-agent-principal-tls on the principal, ?agentname is the agentname, name is agent name only. use 'https://argocd-agent-resource-proxy:9090?agentName=cluster-1'. labels make this show up as an agent to the CLI

...
* `cd principal && kustomize build --enable-helm | kubectl apply -f - --context do-atl1-k8s-1-33-1-principal`
* Run `kustomize build --enable-helm | kubectl apply -f - --context do-atl1-k8s-1-33-1-principal` again since app.yaml failed without the CRDs yet
...
* `kustomize build --enable-helm | kubectl apply -f - --context do-atl1-k8s-1-33-1-cluster-1`
